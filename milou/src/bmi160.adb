...
package body BMI160 is
   ...
   procedure Fail (R : out Status; K : Status_Kind) is
   begin
      R.Kind := K;
   end Fail;
   ...
   begin
      ...
      if not Write_Reg (ACC_RANGE_REG, Encode(Rate)) then
         Fail (Result, Bus_Error);
         return;
      end if;

      if not Write_Reg (GYR_RANGE_REG, Encode (Gyro_R)) then
         Fail (Result, Bus_Error);
         return;
      end if;

      if not Write_Reg (ACC_CONF_REG, Acc_Conf_Value (Rate)) then
         Fail (Result, Bus_Error);
         return;
      end if;

      if not Write_Reg (GYR_CONF_REG, Gyr_Conf_Value (Rate)) then
         Fail (Result, Bus_Error);
         return;
      end if;
